...
The human study used 100 blinded proof pairs: 36 canonical contrasts and 64 stratified corpus pairs, each rated by three topology mathematicians answering the same prompt: "Which proof uses the less standard mathematical route to its result?"

**Rater screening.** Four raters were recruited. The study packet includes repeated pairs — the same two proofs, order sometimes swapped — which support an internal-consistency check: an engaged rater selects the same underlying proof each time. One rater was internally consistent on only 20 of 36 repeat-comparisons, at chance, versus 34/36, 30/36, and 34/36 for the other three, alongside a median response time of roughly 7 seconds per pair against a packet designed for 90–120 seconds of reading. That rater's submissions were excluded prior to any outcome analysis; all results below use the three retained raters.

**Inter-rater agreement (the ceiling).** Pairwise agreement was 79% (Cohen's κ = 0.575), 81% (κ = 0.612), and 74% (κ = 0.467) across the three rater pairs. Three-rater agreement is Fleiss κ = 0.551 overall (0.685 on canonical, 0.476 on stratified), with all three raters unanimous on 67/100 pairs (28/36 canonical, 39/64 stratified). The stratified split has the weakest agreement, so stratified consensus is a moderately noisy target and the corresponding generalization numbers should be read against that ceiling rather than as ground truth.

**PriorProof vs. the majority.** Against the three-rater majority, PriorProof matched 76/100 pairs overall (76.0%, Wilson 95% CI [66.8%, 83.3%]). The split matters: 34/36 on canonical contrasts (94.4%, [81.9%, 98.5%]) versus 42/64 on stratified pairs (65.6%, [53.4%, 76.1%]). The canonical result confirms the metric behaves correctly on clear proof-route contrasts; the stratified result is the honest broad-pair generalization number. Agreement with the three raters individually was 75%, 74%, and 68%.
...
|---|---:|---:|
| Smallest quartile | 15/25 = 60.0% | [40.7%, 76.6%] |
| Second quartile | 17/25 = 68.0% | [48.4%, 82.8%] |
| Third quartile | 20/25 = 80.0% | [60.9%, 91.1%] |
| Largest quartile | 24/25 = 96.0% | [80.5%, 99.3%] |

The point estimates increase monotonically, but the confidence intervals overlap heavily across adjacent bins, so we claim endpoint calibration rather than a resolved four-step staircase. The smallest score-gap quartile does not differ detectably from chance (60.0%, interval includes 50%), while the largest excludes chance by a wide margin (96.0%, [80.5%, 99.3%]). In use, small metric differences should be treated as low-confidence comparisons; large differences are the reliable regime in this sample.

**LLM baseline.** An LLM judge given the identical prompt scores comparably. The best condition, `gpt-5-mini` with the strict prompt, matched the majority on 80/100 pairs (80.0%, [71.1%, 86.7%]); a majority vote over the four LLM conditions (excluding 5 pairs with 2-2 splits across conditions) matched 77/95 non-tied pairs (81.1%, [72.0%, 87.7%]). LLM pairwise judgments are nondeterministic — two independent runs at identical settings differed on 17 of 100 pairs — which is a further reason to treat the LLM-vs-metric gap as unresolved rather than a clean baseline win. PriorProof's 76.0% [66.8%, 83.3%] is lower in point estimate, but its interval overlaps the LLM's substantially, so the two are not separated at this sample size. PriorProof's contribution is therefore not to beat an LLM judge on pairwise comparison, but to provide an annotation-free, mechanical, point-in-time signal whose score gap exposes a reliability knob — high-gap comparisons are trustworthy, low-gap ones are not — that a language-model judge does not expose.

## Limitations

The human study has three raters. Three-rater agreement is moderate (Fleiss κ = 0.551 overall), and the stratified split — where the broad generalization claim lives — is weakest (κ = 0.476), so stratified consensus is a somewhat noisy ceiling rather than ground truth. A fourth recruited rater was excluded for chance-level internal consistency; the reported results are robust to that exclusion in the sense that the retained three agree substantially, but the episode underscores that expert judgment on subtle proof-route contrasts is itself variable. The study covers one Mathlib domain, topology; the retrieval-heavy prior fit, inert threshold sweep, and zero live redundancy hits may be domain-specific, and the all-Mathlib prior grid already shows a different mixture shape. The nested redundancy detector is implemented and fixture-tested but dormant on the live corpus until extraction emits proof subterms. The calibration result is established at the endpoints, not as a statistically resolved four-bin staircase. Finally, the LLM baseline's point estimate exceeds PriorProof's on broad pairwise comparison, and while the difference is within sampling noise at this scale, we do not claim the metric matches or beats a language-model judge as a direct pairwise ranker; its value is the self-calibrating, training-corpus-anchored signal, not pairwise-judgment accuracy.
...
**Validation.** Run all six mechanical tests. Run the threshold sweep with the footprint-bucket diagnostic; only treat it as a sensitivity result when at least some declarations change family buckets across thresholds. The reported topology study used three domain raters and 100 blinded pairwise comparisons: 36 canonical contrasts and 64 stratified corpus pairs. Report rater agreement, the three-rater majority ceiling, PriorProof-vs-majority, and the LLM baseline separately for canonical and stratified pairs, with Wilson intervals on every proportion. For the LLM baseline, report the tie rule (exclude 2-2 splits across the four conditions) and note run-to-run nondeterminism as context for small numerical differences in reproduction.
