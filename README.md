...
**LLM baseline.** An LLM judge given the identical prompt scores comparably. The best condition, `gpt-5-mini` with the strict prompt, matched the majority on 81/100 pairs (81.0%, [72.2%, 87.5%]); a majority vote over the four LLM conditions matched 73/88 non-tied pairs (83.0%, [73.8%, 89.4%]). PriorProof's 76.0% [66.8%, 83.3%] is lower in point estimate, but its interval overlaps the LLM's substantially, so the two are not separated at this sample size. PriorProof's contribution is therefore not to beat an LLM judge on pairwise comparison, but to provide an annotation-free, mechanical, point-in-time signal whose score gap exposes a reliability knob — high-gap comparisons are trustworthy, low-gap ones are not — that a language-model judge does not expose.
